...
\colorlet{brzdimeSoberBg}{brzdimeSober!30}
...
\newcommand{\mbrpSession}[1]{
	...
	\bgroup
		...
		\begin{tcolorbox}[
			...
			colback=white,
			parbox=false,
			adjusted title={\textbf{\Large \arabic{section}. setkání}},
			%frame hidden,
		]
			\vskip1em
			{ \scshape\bfseries\Large \mbrpTitle }
			\vskip1.5em
			\tcbsubtitle{{\large \textbf{Shrnutí}}}
			\vskip.5em
		...
		\begin{tcolorbox}[
			...
			boxrule=.4pt,
			%frame hidden,
			title={Formální cvičení}, %, plánovaná},
			parbox=false,
			coltitle=black,
			colbacktitle=brzdimeBlueBg,
			colback=white,
...
\newcommand{\soberSpace}[1]{
	...
	%\node(top)[text width=.9\linewidth,minimum width=.9\linewidth, text badly centered, align=center]{ \hfill Title };
	...
	\def\pagePracticeLog{
		...
		\subsection{Záznam cvičení \normalPencilLeftDown}
			\ifplastex
				...
				\end{tabular}
			\else
				\def\myStrut{\rule{0pt}{\dimexpr(\pagewidth-8em-2cm-6em-3pt)/7}} % {.07\textheight}}
				\footnotesize
